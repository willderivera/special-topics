...
class graph
{
  public:
   ...
   graph twoOptNeighbor(double time);
   graph threeOptNeighbor(double time);
...
//added funtion to calculate number of conflicts in graph
//for use in this class and also p1b.cpp
int graph::numConflicts() {
   int numConflicts = 0;
...
void graph::printSolution(ofstream &fout)
// Prints a graph coloring solution
//prints to file
{
...
//method computes conflicts for assigning each possible color to given node
//returns lowest color with minimum number of conflicts
int graph::getfirstAvailColor(int node, int numColors) {
	//vector stores conflicts where index is color
	//and value is number of conflicts that it would cause
	vector<int> colors(numColors + 1, 0);

	//set default best color to 1
	int minColor = 1;

	//iterate through all neighboring nodes and increment conflict value
	//for the color of each neighbor
	for (int n = 0; n < numNodes(); n++) {
		if (isEdge(n, node) || isEdge(node, n)) {
			colors[getColor(n)]++;
		}
	}
...
graph graph::twoOptNeighbor(double time) {
	bool currTime = true;
	graph best = *this;
	graph temp = *this;
	int c1, c2;
	for (int i = 0; i < numNodes() - 1 && currTime; i++) {
		for (int j = i + 1; j < numNodes() && currTime; j++) {
			c1 = getColor(i);
			c2 = getColor(j);
			temp.setColor(i, c2);
			temp.setColor(j, c1);
			...
			temp.setColor(i, c1);
			temp.setColor(j, c2);
			currTime = (double) (clock() / CLOCKS_PER_SEC) < time;
		}
	}
	return best;
}
...
//returns best neighbor
graph graph::threeOptNeighbor(double time) {
	bool currTime = true;
	graph best = *this;
	graph temp = *this;
	int c[3];
	...
	int aj[] = {2, 2, 0, 0, 1, 1};
	int ak[] = {1, 0, 2, 1, 0, 2};
	for (int i = 0; i < numNodes() - 2 && currTime; i++) {
		for (int j = i + 1; j < numNodes()- 1 && currTime; j++) {
			for (int k = j + 1; k < numNodes() && currTime; k++) {
				c[0] = temp.getColor(i);
				c[1] = temp.getColor(j);
				c[2] = temp.getColor(k);
				for (int x = 0; x < 6; x++) {
					temp.setColor(i, c[ai[x]]);
					temp.setColor(j, c[aj[x]]);
					temp.setColor(k, c[ak[x]]);
					if(temp.numConflicts() < best.numConflicts()) {
						best = temp;
					}
				}
